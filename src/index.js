...
function findAllPSiblings(where) {
    ...
    var chldrn = where.children;

    for (var i = 0; i < chldrn.length - 1; i++) {
        if (chldrn[i].nextElementSibling.tagName === 'P') {
            arrayOfElems.push(chldrn[i]);
        }
    }
...
function deleteTextNodesRecursive(where) {
    var ch = where.childNodes;

    for (var i = 0; i < ch.length; i++) {
        if (ch[i].nodeType === Node.TEXT_NODE) {
